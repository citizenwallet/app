...
class Ecies {
  ...
  ///
  // List<int> AESEncrypt(List<int> messageBuffer, SVPrivateKey senderPrivateKey,
  //     SVPublicKey recipientPublicKey) {
  ...
  //   final pubkeyS =
  //       SVPublicKey.fromXY(S.x!.toBigInteger()!, S.y!.toBigInteger()!);
  ...
  ///
  List<int> AESDecrypt(List<int> cipherText, Uint8List privateKey) {
    ...
    // final ECPoint S = (senderPublicKey.point *
    ...
    final pubkeyHash = SHA512Digest().process(privateKey);
    ...
    if (!const ListEquality().equals(messageChecksum, hmac)) {
    ...
    final decrypted = decryptionCipher.process(
        cipherText.sublist(37, cipherText.length - tagLength) as Uint8List);
    return decrypted;
  }
}
